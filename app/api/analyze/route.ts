...
`;
    ...
    if (classification.type !== "estimate" && classification.type !== "flyer") {
      ...
      if (classification.type === "face") {
        ...
        const emojiReasons = [
          "柔らかい雰囲気と芯の強さが同居していて、周りに安心感を与えるタイプです。小さなことにも前向きに取り組む姿勢が魅力的で、だからこの絵文字になるんだよ。",
          "穏やかな空気感の中にワクワク感があって、近くにいる人の気分を明るくできるムードです。自然体なのに華があるので、だからこの絵文字になるんだよ。",
          "落ち着きと行動力のバランスが良く、どんな場面でも自分のペースを保てる印象です。やさしさが滲み出ているから、だからこの絵文字になるんだよ。",
          "人を惹きつける温かさと、前向きに進める強さが見えてきます。周囲を元気づけるパワーがあるので、だからこの絵文字になるんだよ。",
          "安心感のある表情に加えて、挑戦を楽しめるエネルギーが伝わってきます。自然と運が集まるタイプだから、だからこの絵文字になるんだよ。",
          "柔らかな雰囲気と芯の通った意志が感じられ、周囲に信頼を生むタイプです。あなたらしさが際立つので、だからこの絵文字になるんだよ。"
        ];

        const emoji = emojiCandidates[Math.floor(Math.random() * emojiCandidates.length)];
        const reason = emojiReasons[Math.floor(Math.random() * emojiReasons.length)];
        ...
        return NextResponse.json({
          result: {
            ...
            emoji_char: emoji,
            emoji_reason: reason,
            fortune_title: "あなたの絵文字診断",
            fortune_subtitle: "あなたの雰囲気から導いた絵文字",
            fortune_summary: reason,
            has_unconfirmed_items: false,
            unconfirmed_item_names: [],
          }
        });
      }
