...
以下の優先順位1〜4に従って、機械的に判定を行ってください。

### 優先順位1：【ターゲット項目】（無条件で判定）
| 項目名 | 判定 (status) | 適正価格 (price_fair) | 理由 (reason) |
| :--- | :--- | :--- | :--- |
| **仲介手数料** | **negotiable** | 賃料の0.55ヶ月分 | "原則は0.5ヶ月分です。交渉により減額、または貸主負担（AD）により0円にできる可能性があります。" |
| **火災保険** | 2万円超なら **negotiable** | 15,000〜20,000 | "相場より割高です。指定が高い場合、自分で安い保険（借家人賠償責任保険）に加入可能です。" |

### 優先順位2：【ブラックリスト項目】（徹底的に疑う）
対象キーワード: **消毒, 抗菌, 害虫駆除, サポート, 24時間, 安心入居, 消火剤, 消火器, 事務手数料, 書類作成, クラブ会費**

1. **図面に記載がない場合**
   - 判定: **cut** (0円)
   - 理由: "**募集図面に記載がないため、支払い義務のない項目です。全額カット可能です。**"
2. **図面に記載があるが「任意」「オプション」とある場合**
   - 判定: **cut** (0円)
   - 理由: "**任意のオプションサービスです。不要であれば外せます。**"
3. **図面に記載があり、必須等の表記がある場合**
   - 判定: **negotiable** (0円)
   - 理由: "**図面に記載がありますが、交渉次第で外せる可能性が高い項目です。**"

### 優先順位3：【ホワイトリスト項目】（大家・管理会社の設定）
対象: **敷金, 礼金, 賃料, 前家賃, 共益費/管理費, 更新料, 保証会社利用料, 鍵交換代, 町内会費, 口座振替手数料, クリーニング費**

1. **基本ルール**
   - 見積書に記載があれば、**図面から読み取れなくても "fair"** と判定する。（AIの読取ミスによる誤判定を防ぐため）
   - 理由: "**物件固有の条件であり、適正な費用です。**"
2. **例外（Cutにするケース）**
   - 図面に明確に**「礼金0円」「礼金なし」「鍵交換代無」**という文字が読み取れた場合に限り、見積書に金額があれば **cut** とする。

### 優先順位4：【その他】
- 上記に該当しない項目は、原則 **fair** とする。
...

{
  ...
  "items": [
    {
      ...
      "reason": "上記の判定ルールに基づいた具体的で強い口調のアドバイス",
  ...
  "discount_amount": 差額,
  "pro_review": { 
    "content": "この物件の初期費用における最大の問題点と、次に取るべき具体的なアクション（仲介手数料の交渉やオプション削除の依頼など）を一言で。" 
  },
  "risk_score": 0〜100（削減額の割合に応じて算出）
}
`;
