...
const DIAGNOSIS_STORAGE_KEY = 'diagnosis_result';
  ...
  useIsomorphicLayoutEffect(() => {
    try {
      ...
      if (parsed && typeof parsed === 'object' && Array.isArray(parsed.items)) {
        ...
        // LINE連携トークンも復元
        const savedLineToken = sessionStorage.getItem('diagnosis_line_token');
      ...
      try {
        sessionStorage.removeItem(DIAGNOSIS_STORAGE_KEY);
        sessionStorage.removeItem('diagnosis_line_token');
      } catch {
        // 無視
      }
    }
    ...
    try {
      ...
      setTimeout(async () => {
        ...
        try {
          ...
          if (caseRes.ok) {
            const { caseToken, token } = await caseRes.json();
            const resolvedToken = caseToken || token || null;
            if (resolvedToken) {
              setLineToken(resolvedToken);
              sessionStorage.setItem('diagnosis_line_token', resolvedToken);
            }
          }
        } catch {
                    ...
                    <div className="flex items-center justify-center gap-4">
                      ...
                      {((result.discount_amount ?? calculateDiscountAmount(result.items)) > 0) ? (
                        <div className="flex flex-col text-left leading-tight pointer-events-none">
                          <span className="text-sm md:text-lg font-bold text-white drop-shadow-md underline decoration-2 decoration-black underline-offset-[1px] inline-block">
                            <span className="text-[#ff0000] font-extrabold text-base md:text-xl">割引済み</span><span className="text-white">の見積もりを</span>
                          </span>
                          <span className="text-base md:text-xl font-extrabold text-white drop-shadow-md">無料で確認する</span>
                        </div>
                      ) : (
